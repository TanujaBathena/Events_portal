...
const FilesUploader=(props)=>{
    ...
    const wrongFileExtensions=(filesList)=>{
        ...
        const types = ['image/png', 'image/jpeg', 'image/gif','application/pdf','application/doc','application/docx']
        // loop access array
    ...
    const onNewFiles=(filesList)=>{
        ...
        if(NoOfFilesValidation(filesList.length) && wrongFileExtensions(filesList) && harmfulFiles(fileList) && checkFileSize(filesList)){
            filesList.map((file)=>file.id=uuid());
            //let newList=fileList;
            if(fileList.length===0) {updateFileList(filesList);console.log("Inside if ",filesList.length," ", fileList.length)}
            else{
                let arr=[];
                ...
                for(let i=0;i<filesList.length;i++)
                {
                    ...
                    for(let j=0;j<fileList.length;j++){
                        if(filesList[i].name===fileList[j].name){isThere=true;break;}
                    }
                    if(!isThere){
    ...
    return(
        <div className="attribute" style={{flexDirection:"column",textAlign:"center"}}>
            <label htmlFor="files" style={{width:'100%',fontSize:'x-large'}}>Add files related to the internship :</label>
            <div style={{display:"flex",flexDirection:'column',width:"100%"}}>
            <input type="file" title=" " id="files" name="files" multiple={true} accept="image/*,.pdf,.doc,.docx" onChange={(e)=>{onNewFiles(Array.from(e.target.files));console.log("Hiiiiii")}} style={{height:"25px",margin:"auto",width:'25%'}}/>
        ...
        <div>
            ...
            <form className="form" onSubmit={onSubmitHandler} className="form">
                <div className="attribute">
                    ...
                    <input type="text" id="intTitle" name="intTitle" value={internshipTitle} onChange={
                        (e)=>{
                            // if(lengthValidation(internshipTitle,5)){
                ...
                <div className="attribute">
                    ...
                    <input type="text" id="company" name="company" value={company} onChange={(e)=>{setCompany(e.target.value)}}style={{width:"70%",height:"4vh"}} />
                </div>

                <div className="attribute">
                    <label htmlFor="stipend">Stipend  </label>
                    <input type="text" id="stipend" name="stipend" value={stipend} onChange={(e)=>{setStipend(e.target.value)}}style={{width:"70%",height:"4vh"}} />
                </div>

                <div className="attribute" style={{boxSizing:'border-box'}}>
                              <label htmlFor="branches"style={{width:"20%",height:"4vh",marginTop:'3%'}}>Branches  </label>
                    {/* <div>
                        <div>
                    ...
                    <div style={{width:"70%",marginTop:'auto'}}>
                        <Dropdown selected={selected} setSelected={setSelected} style={{padding:'0px'}} />
                ...
                <div className="attribute">
                    ...
                    <input type="datetime-local" id="deadline" name="deadline" value={date} onChange={(e)=>setDate(e.target.value)}style={{width:"70%",height:"4vh"}} />
                    {console.log(date)}
                ...
                <div className="attribute">
                    ...
                    <textarea name="description" id="description" cols="30" rows="10" value={description} onChange={(e)=>{setDescription(e.target.value)}} style={{width:"70%",height:"4vh"}}></textarea>
                </div>
                
                <FilesUploader fileList={fileList} updateFileList={updateFileList} style={{display:"flex"}}/>
